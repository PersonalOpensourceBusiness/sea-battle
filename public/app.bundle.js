	...
	var Model = exports.Model = function () {
	    ...
	    _createClass(Model, [{
	        ...
	        value: function init() {
	            ...
	            this.socket = new _Socket.Socket();
	            this.socket.init();
	        ...
	        key: 'attack',
	        value: function attack(board, y, x) {
	            this.socket.attack(board, y, x);
	        }
	    }, {
	        key: 'addShip',
	        value: function addShip(matrix, ship) {
	        ...
	        key: 'checkMatrix',
	        value: function checkMatrix(y, x) {
	        ...
	        key: 'getReady',
	        value: function getReady(player) {
	...
	var Socket = exports.Socket = function () {
	    ...
	    _createClass(Socket, [{
	        ...
	        value: function init() {
	            ...
	            this.socket.on('step2', function (data) {
	                _this.rooms = data;
	            });
	            this.socket.on('pushMatrix', function (data) {
	                console.log(data);
	                _this.data = data;
	            });
	        }
	        ...
	        key: 'pushMatrix',
	        value: function pushMatrix(matrix) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Ship = __webpack_require__(55);
	...
	var View = exports.View = function () {
	    ...
	    _createClass(View, [{
	        key: 'createBoard',
	        value: function createBoard(className, id) {
	            return new _Board.Board(this.app, this.model.createBoard().matrix, className, id);
	        }
	    }, {
	        key: 'createInterface',
	        ...
	        value: function renderChoice() {
	            ...
	            document.querySelector('#players').addEventListener('keyup', function (e) {
	        ...
	        key: 'renderGameList',
	        value: function renderGameList(rooms) {
	            this.interface.render(this.interface.join(rooms));
	        }
	    }, {
	        key: 'moveShip',
	        value: function moveShip(x, y, board, ship, hor) {
	            return _Ship.Ship.move(x, y, board, ship, hor);
	        }
	    }, {
	        key: 'removeShip',
	        value: function removeShip(x, y, board, ship, hor) {
	            return _Ship.Ship.remove(x, y, board, ship, hor);
	        }
	    }, {
	        key: 'createShip',
	        value: function createShip(x, y, board, ship, hor) {
	            var newShip = _Ship.Ship.create(x, y, board, ship, hor);
	            if (newShip) {
...
/***/ function(module, exports) {
	...
	var Interface = exports.Interface = function () {
	    ...
	    _createClass(Interface, [{
	        ...
	        key: 'main',
	        value: function main() {
	            return ['<div class="interface">', '<h1>Sea battle</h1>', '<hr>', '<div>', '<button id="create">Создать игру</button>', '<button id="bot">Создать игру с ботом</button>', '<button id="join">Присоединиться к игре</button>', '</div>', '</div>'].join('');
	        }
	    }, {
	        key: 'choice',
	        value: function choice() {
	            return ['<div class="interface">', '<h1>Sea battle</h1>', '<hr>', '<div>', 'Сколько игроков?', '<input type="number" max="20" min="2" value="2" id="players">', '<button id="init">Init game</button>', '</div>', '</div>'].join('');
	        }
	    }, {
	        key: 'join',
	        value: function join(room) {
	            ...
	            return ['<div class="interface">', '<h1>Sea battle</h1>', '<hr>', '<div>', 'Список комнат', '<ul class="interface__view">', result.join(''), '</ul>', '<button id="join-game">Join game</button>', '</div>', '</div>'].join('');
	        }
	    }, {
	        key: 'wait',
	    ...
	    _createClass(Ship, null, [{
	        ...
	        value: function move(x, y, board, ship, hor) {
	            ...
	            if (hor) {
	                ...
	                for (var _j = y - i; _j < y - i + ship.size; _j++) {
	                    matrix.push([x, _j]);
	                    if (row[_j]) row[_j].querySelectorAll('.cell')[x].classList.add('hover');
	                }
	                return matrix;
	            }
	        }
	        ...
	        key: 'remove',
	        value: function remove(x, y, board, ship, hor) {
	            var row = board.querySelectorAll('.row');
	            var i = Math.floor(ship.size / 2);
	            var matrix = [];
	            ...
	            if (hor) {
	                ...
	                if (ship.size > 2 && (y < i || y >= row.length - 1 || y % 2 !== 0 && y + i >= row.length) || ship.size === 2 && y < (y < ship.size || y >= row.length - 1)) return;
	                for (var _j2 = y - i; _j2 < y - i + ship.size; _j2++) {
	                    matrix.push([x, _j2]);
	                    if (row[_j2]) row[_j2].querySelectorAll('.cell')[x].classList.remove('hover');
	                }
	                return matrix;
	            }
	        }
	        ...
	        key: 'create',
	        value: function create(x, y, board, ship, hor) {
	            var row = board.querySelectorAll('.row');
	            if (!ship) return;
	            var i = Math.floor(ship.size / 2);
	            var matrix = [];
	            if (hor) {
	                ...
	                if (ship.size > 2 && (y < i || y >= row.length - 1 || y % 2 !== 0 && y + i >= row.length) || ship.size === 2 && y < (y < ship.size || y >= row.length - 1)) return;
	                for (var _j3 = y - i; _j3 < y - i + ship.size; _j3++) {
	                    matrix.push([x, _j3]);
	                    if (row[_j3]) row[_j3].querySelectorAll('.cell')[x].classList.add('ship');
	                }
	            }
	...
	var Controller = exports.Controller = function () {
	    ...
	    _createClass(Controller, [{
	        ...
	        value: function init() {
	            ...
	            this.model.socket.socket.on('attack', function (data) {
	                Array.prototype.forEach.call(document.querySelectorAll('.field'), function (item) {
	        ...
	        value: function eventHandlers() {
	            ...
	            Object.keys(this.players).map(function (item) {
	                ...
	                board.addEventListener('mouseover', function (e) {
	                    return _this.onHover(e, _this.view.moveShip, board, item);
	                });
	                board.addEventListener('mouseout', function (e) {
	                    return _this.onHover(e, _this.view.removeShip, board);
	                });
	                board.addEventListener('click', function (e) {
	                    return _this.onClick(e, board, item);
	                });
	            });
	            this.model.socket.socket.on('ready', function () {
	                for (var i = 0; i < _this.model.socket.data[_this.player].length; i++) {
	                    for (var j = 0; j < _this.model.socket.data[_this.player][i].length; j++) {
	                        _this.count += _this.model.socket.data[_this.player][i][j];
	                    }
	                }
	                _this.ready = true;
	            });
	            this.model.socket.socket.on('move:ready', function () {
	                return _this.ready = true;
	            });
	            document.querySelector('.horizontal-button').addEventListener('click', function () {
	                _this.horizontal = !_this.horizontal;
	                document.querySelector('.horizontal-button').innerText = _this.horizontal ? 'Горизонтально' : 'Вертикально';
	            });
	        }
	    }, {
	        key: 'clearInterface',
	        ...
	        value: function startNewGame(players) {
	            ...
	            players.map(function (item) {
	                ...
	                item === _this2.player ? _this2.players[_this2.player].board.board.dataset.key = _this2.player : _this2.players[item].board.board.dataset.key = item;
	                item === _this2.player ? _this2.players[_this2.player].board.board.classList.add(_this2.player) : _this2.players[item].board.board.classList.add(item);
	                _this2.bot = item === 'bot';
	            });
	        ...
	        value: function renderInterface() {
	            ...
	            document.querySelector('#create').addEventListener('click', function () {
	                ...
	                document.querySelector('#init').addEventListener('click', function () {
	                    ...
	                    _this3.model.createGame(document.querySelector('#players').value);
	                    _this3.view.renderWait(document.querySelector('#players').value);

	                    // Подсвечиваем иконку с игроком
	                    _this3.model.socket.socket.on('create', function (data) {
	                        ...
	                        _this3.playersCount = data.players;
	                    });
	                });
	            });
	            document.querySelector('#bot').addEventListener('click', function () {
	                ...
	                document.querySelector('#init').addEventListener('click', function () {
	                    ...
	                    _this3.model.socket.socket.on('create', function (data) {
	                        _this3.clearInterface();
	                        _this3.startNewGame([_this3.model.socket.getId(), 'bot']);
	                    });
	                });
	            });
	            document.querySelector('#join').addEventListener('click', function () {
	                ...
	                _this3.model.socket.socket.on('join', function (data) {
	                    _this3.view.renderGameList(data.roomsID);
	                    Array.prototype.forEach.call(document.querySelectorAll('.interface-js-item'), function (item) {
	                        ...
	                        item.addEventListener('click', function () {
	                            ...
	                            document.querySelector('#join-game').addEventListener('click', function () {
	                                _this3.model.connect(item.dataset.room);
	                            });
	                        });
	                    });
	                });
	            ...
	            this.model.socket.socket.on('connected', function (data) {
	                var i = 0;
	                _this3.view.renderWait(data.playersCount);

	                // Выводим зеленые иконки при подключении человека
	                for (var key in data.rooms[data.roomsID].sockets) {
	                ...
	                document.querySelector('#start').addEventListener('click', function () {
	                    ...
	                    for (var _i = 0; _i < Object.keys(_this3.model.socket.rooms.rooms.sockets).length; _i++) {
	                        arr[_i] = Object.keys(_this3.model.socket.rooms.rooms.sockets)[_i].substr(2);
	                    }
	                    _this3.clearInterface();
	        ...
	        key: 'onHover',
	        value: function onHover(e, action, board, item) {
	            ...
	            if (board.classList.contains('your')) {
	                ...
	                if (!Object.keys(this.model.ships).length) {
	                    if (this.bot) {
	                        if (this.i > 0) {
	                            ...
	                            for (var i = 0; i < this.model.socket.data[this.player].length; i++) {
	                                for (var j = 0; j < this.model.socket.data[this.player][i].length; j++) {
	                ...
	                var matrix = action(x, y, board, ship, this.horizontal);
	                if (!matrix) return;
	                var line = [];

	                for (var _i2 = 0; _i2 <= matrix.length - 1; _i2++) {
	                    line.push(this.model.field.matrix[matrix[_i2][1]][matrix[_i2][0]]);
	                }
	                this.empty = line.every(function (elem) {
	                    return elem === 0;
	                });
	        ...
	        key: 'onClick',
	        value: function onClick(e, board, item) {
	            var x = Array.prototype.indexOf.call(e.target.parentNode.querySelectorAll('.cell'), e.target);
	            var y = Array.prototype.indexOf.call(board.querySelectorAll('.row'), e.target.parentNode);
	            var ship = void 0;
